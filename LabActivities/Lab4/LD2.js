...
for (var i = 0; i < solidCubeLookups.length; i++)
{
   ...
   shapes.solidCube.normals.push(vec3(0,0,1));
...
function loadShape(myShape, type)
{
   ...
   if(type == gl.TRIANGLES){
        //calculate normals
        makeFlatNormals(points, myShape.start, myShape.size, normals);
   }
...
function render()
{
    ...
    //gl.uniform4fv(light.position,vec4(0.0, 0.0, 1.0, 0.0));//if the 4th argument is 0 then that is vector, 1 then point(?)
    //gl.uniform4fv(light.position,vec4(0.0, 1.0, 0.0, 0.0));//if the 4th argument is 0 then that is vector, 1 then point(?)
    //gl.uniform4fv(light.position,vec4(0.0, 1.0, 0.0, 1.0));//if the 4th argument is 0 then that is vector, 1 then point(?)
    //gl.uniform4fv(light.position,vec4(0.0, 1.0, -10.0, 1.0));//if the 4th argument is 0 then that is vector, 1 then point(?)

    //Set up some default material properties
    //gl.uniform4fv(material.diffuse, vec4(0.8, 0.8, 0.8, 1.0));
    gl.uniform4fv(material.diffuse, vec4(lightblue));
    gl.uniform4fv(material.ambient, mult(vec4(0.5, 0.5, 0.5, 1.0),lightblue));
    ...
    mv = lookAt(eye,at,up);
    //var lpos = vec4(0.0, 1.0, 0.0, 1.0);//if the 4th argument is 0 then that is vector, 1 then point(?)
    var lpos = vec4(0.0, 1.0, 0.0, 0.0);//if the 4th argument is 0 then that is vector, 1 then point(?)
    //var lpos = vec4(1.0, 0.0, 0.0, 1.0);//if the 4th argument is 0 then that is vector, 1 then point(?)
    ...
    gl.uniform4fv(material.diffuse, lightgreen); //ADDED
    gl.uniform4fv(material.ambient, mult(vec4(0.5, 0.5, 0.5, 1.0),lightgreen));//   added
